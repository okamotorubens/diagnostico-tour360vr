...
def gerar_pdf_bytes(dados):
    ...
    if reviews_count < 30:
        pdf.cell(w_card - 4, 3.5, clean_txt(f"Apenas {reviews} avaliações"))
    else:
    ...
    itens = [
        ("Completude do Cadastro", "Site cadastrado" if website else "Sem site próprio cadastrado.", "Perfil incompleto reduz a conversão de novos clientes."),
        ("Nota e Avaliações", f"Nota {rating} baseada em {reviews} avaliações.", "Reputação vulnerável; base pequena limita prova social."),
        ("Consistência de NAP", "Dados de endereço e telefone ativos.", "Informações corretas evitam perdas por buscas."),
        ("Categorias", "1 categoria cadastrada.", "Falta de categorias secundárias limita a visibilidade."),
        ("Fotos", f"Apenas {photos_count} fotos (Cobertura baixa).", "Poucas fotos impedem a avaliação do espaço."),
        ("Horários", f"Horários: {has_hours}.", "Informação correta evita perda de clientes."),
        ("Presença Imersiva", "Nenhum Tour 360° interativo.", "Perdem-se conversões por falta de experiência 360."),
        ("Prova Social Orgânica", "Baixo volume de fotos de clientes.", "Falta de registros reais reduz a confiança do público.")
    ]
