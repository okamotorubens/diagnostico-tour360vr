...
# Título e Subtítulo alinhados conforme pedido
...
def gerar_pdf_bytes(dados):
    ...
    for i, (title, val, label, sc, footer_txt) in enumerate(boxes):
        ...
        if i == 1:
            desenhar_estrelas_destaque(pdf, 12 + (i * w_card), y_cards + 11.5, rating_raw)
        elif i == 0:
    ...
    # Tabela e Plano (Mantendo a estrutura do anterior...)
    # [O restante da função permanece igual ao código anterior para garantir funcionalidade]
    
    # ... (omitido por brevidade, usar a mesma lógica anterior para Tabela/Plano)
    
    # Frase Final
    pdf.ln(8)
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(20, 50, 135)
    # Subida de 1 linha centralizada
