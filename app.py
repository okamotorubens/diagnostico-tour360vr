...
def clean_txt(txt):
    """Trata acentos e caracteres UTF-8 para o FPDF"""
    if not txt:
        return ""
...
def buscar_dados_google(empresa, cidade, key):
    ...
    # Busca Concorrentes do MESMO segmento (usando a primeira palavra do nome ou tipo principal)
...
class PDFExecutivo(FPDF):
    ...
    def header(self):
        # Tarja Azul Superior (Sólida e Preenchida)
        self.set_fill_color(2, 132, 199)  # Azul Médio #0284c7
        ...
        # Título à direita do cabeçalho
        self.set_font("Helvetica", "B", 9)
        self.set_text_color(255, 255, 255)
        self.set_xy(130, 5)
        self.cell(70, 5, clean_txt("PINCHECK AUDIT"), align="R")
    ...
    def footer(self):
        ...
        self.cell(
            0,
            6,
            clean_txt("WhatsApp: (16) 99133-2121   |   https://tour360vr.com.br/"),
            align="C",
...
def gerar_pdf_bytes(dados, concorrentes):
    ...
    pdf.rect(10, pdf.get_y(), W, 14, "F")
    pdf.set_draw_color(2, 132, 199)
    pdf.line(10, pdf.get_y(), 10, pdf.get_y() + 14)
    ...
    pdf.rect(10, y_cards, 60, 18, "DF")
    pdf.set_font("Helvetica", "B", 6.5)
    pdf.set_text_color(71, 85, 105)
    pdf.set_xy(12, y_cards + 2)
    pdf.cell(56, 3, clean_txt("OTIMIZACAO DO PERFIL"))
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(2, 132, 199)
    pdf.set_xy(12, y_cards + 5.5)
    pdf.cell(56, 5, f"{score}/100")
    # Termômetro Visual
    pdf.set_fill_color(226, 232, 240)
    pdf.rect(12, y_cards + 11.5, 56, 2.5, "F")
    pdf.set_fill_color(2, 132, 199)
    pdf.rect(12, y_cards + 11.5, (56 * score / 100), 2.5, "F")

    # Box 2: Nota
    pdf.rect(75, y_cards, 60, 18, "DF")
    pdf.set_font("Helvetica", "B", 6.5)
    pdf.set_text_color(71, 85, 105)
    pdf.set_xy(77, y_cards + 2)
    pdf.cell(56, 3, clean_txt("NOTA E REPUTACAO"))
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(2, 132, 199)
    pdf.set_xy(77, y_cards + 5.5)
    pdf.cell(56, 5, clean_txt(f"★ {rating}"))
    pdf.set_font("Helvetica", "", 6.5)
    pdf.set_text_color(51, 65, 85)
    pdf.set_xy(77, y_cards + 11)
    pdf.cell(56, 4, clean_txt(f"Com base em {reviews} avaliacoes"))

    # Box 3: Tour 360
    pdf.rect(140, y_cards, 60, 18, "DF")
    pdf.set_font("Helvetica", "B", 6.5)
    pdf.set_text_color(71, 85, 105)
    pdf.set_xy(142, y_cards + 2)
    pdf.cell(56, 3, clean_txt("TOUR VIRTUAL 360°"))
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(220, 38, 38)
    pdf.set_xy(142, y_cards + 5.5)
    pdf.cell(56, 5, clean_txt("0 FOTOS"))
    pdf.set_font("Helvetica", "", 6.5)
    pdf.set_text_color(100, 116, 139)
    pdf.set_xy(142, y_cards + 11)
    pdf.cell(56, 4, clean_txt("Ausencia de midia imersiva"))

    pdf.set_y(y_cards + 22)
    ...
    pdf.cell(42, 5, clean_txt(" Dimensão"), fill=True)
    pdf.cell(73, 5, clean_txt(" Estado Atual Identificado"), fill=True)
    pdf.cell(
        75,
        5,
    ...
    itens = [
        (
            ...
            "Website cadastrado"
            if website
            else "Faltam descrição, site próprio ou dados.",
            "Perfil incompleto transmite falta de profissionalismo e reduz a conversao.",
        ),
        (
            "Nota e Avaliações",
            f"Nota {rating} com {reviews} avaliacoes acumuladas.",
            "Reputacao ativa fortalece a prova social e gera confianca imediata.",
        ),
        (
            "Consistência de NAP",
            "Endereco e telefone ativos no Google Maps.",
            "Informacoes corretas garantem confianca ao algoritmo e evitam buscas frustradas.",
        ),
        (
            "Categorias",
            "Categoria principal definida na ficha.",
            "Falta de categorizacoes secundarias limita a visibilidade em buscas locais.",
        ),
        (
            "Fotos",
            f"{photos_count} fotos identificadas no perfil.",
            "Poucas fotos impedem que os visitantes avaliem a estrutura antes da visita.",
        ),
        (
            "Horários",
            f"Horários de funcionamento: {has_hours}.",
            "Informacao correta evita perda de atendimentos em horarios de pico.",
        ),
        (
            "Posts / Novidades",
            "Nenhum post recente detectado.",
            "Perfil estatico nao incentiva revisitas e deixa de destacar promocoes.",
        ),
        (
            "Recursos Interativos",
            "Nenhuma foto 360° ou tour virtual detectado.",
            "Visitantes nao conseguem 'visitar' o local; perdem-se conversoes por falta de imersao.",
        ),
        (
            "Presença e Concorrência",
            f"Concorrentes no ramo: {txt_conc}",
            "Como voce esta na regiao -- e onde esta a oportunidade de supera-los.",
        ),
    ]
    ...
    for i, (dim, est, imp) in enumerate(itens):
        ...
        pdf.set_text_color(3, 105, 161)
        pdf.cell(42, 4.5, clean_txt(f" {dim}"), fill=True)
        pdf.set_text_color(51, 65, 85)
        pdf.cell(73, 4.5, clean_txt(f" {est[:48]}"), fill=True)
    ...
    acoes = [
        (
            ...
            "Mapeamento imersivo em alta definicao integrado ao Google Maps. Aumenta o tempo de permanencia na ficha e multiplica os agendamentos.",
        ),
        (
        ...
        (
            ...
            "Reestruturacao completa de palavras-chave, categorias e estrategia para alavancar avaliacoes de clientes satisfeitos.",
        ),
    ]
    ...
    for tit, desc in acoes:
        ...
        pdf.rect(10, pdf.get_y(), W, 8.5, "DF")

        pdf.set_font("Helvetica", "B", 7.5)
        pdf.set_text_color(3, 105, 161)
        pdf.set_xy(12, pdf.get_y() + 1)
        pdf.cell(0, 3, clean_txt(tit), new_x="LMARGIN", new_y="NEXT")

        pdf.set_font("Helvetica", "", 6.5)
        pdf.set_text_color(51, 65, 85)
        pdf.set_x(12)
        pdf.cell(0, 3, clean_txt(desc), new_x="LMARGIN", new_y="NEXT")
        pdf.set_y(pdf.get_y() + 1.5)

    pdf.ln(2)

    # Chamada para Ação (CTA Unificado em Bloco Único)
    pdf.set_fill_color(240, 249, 255)
    pdf.set_draw_color(2, 132, 199)
    pdf.rect(10, pdf.get_y(), W, 13, "DF")
